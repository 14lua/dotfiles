using Plots
using LinearAlgebra  # Import LinearAlgebra to use the norm function
...
velocities1 = zeros(num_steps, 2)
velocities2 = zeros(num_steps, 2)
...
for i in 1:num_steps-1
    ...
    distance = norm(r)  # Use norm from LinearAlgebra
    ...
    velocities1[i+1, :] = velocities1[i, :] + force_vector / m1 * dt
    velocities2[i+1, :] = velocities2[i, :] - force_vector / m2 * dt
    
    # Update positions
    positions1[i+1, :] = positions1[i, :] + velocities1[i+1, :] * dt
    positions2[i+1, :] = positions2[i, :] + velocities2[i+1, :] * dt
end
